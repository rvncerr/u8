...
main(int argc, char** argv)
{
  ...
  if (NULL == CU_add_test(pSuite, "u8_rune_length", test_u8_rune_length)) {
    CU_cleanup_registry();
    return CU_get_error();
  }

  if (NULL == CU_add_test(pSuite, "u8_find_rune", test_u8_find_rune)) {
    CU_cleanup_registry();
    return CU_get_error();
  }
  ...
  if (NULL == CU_add_test(pSuite, "u8_loop", test_u8_loop)) {
    CU_cleanup_registry();
    return CU_get_error();
  }
